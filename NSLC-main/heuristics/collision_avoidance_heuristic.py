...
class CAHeuristic(Heuristic):
    ...
    def step(self):
        ...
        if camera_dist[1] < 0.25:  # if we see something on our right
            self.robot.set_rotation(self.rot_speed)  # turn left
        elif camera_dist[2] < 0.25:  # or in front of us
            self.robot.set_rotation(self.rot_speed) #avoid it
        elif camera_dist[3] < 0.25:  # Otherwise, if we see something on our left
